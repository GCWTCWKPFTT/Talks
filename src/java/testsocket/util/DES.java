...
public class DES { 
    private static int [][]key = new int[17][48];  
  ...
  //S盒矩阵  
	...
	private static byte[] re_byte(String str) throws UnsupportedEncodingException {
	...
	private static void ASCII2Bit(byte bit[], byte des[]) {// 转换为二进制
	...
	private static  String bit2Hex(byte[]a){  
        String strRtn = "";  
        int length = a.length;  
        int i=0,value=0;  
	  ...
	  private static void bit2Byte(byte[] a,byte[] b){  
	 ...
	 private static void hex2Bit(String hex,byte []bit){  
	        String strTmp ="";  
	        int i = 0;  
	...
	private static byte[] permute_ip(byte[] b) {// 置换ip
		byte a[] = new byte[64];
	  ...
	  private static void F_Sbox(int n, byte[] ll, byte[] rr, byte[] LL, byte[] RR){  
	        int i, j,y,z,k,m;  
	...
	private byte[] conversion_e(byte[] b) {// E扩展
		byte a[] = new byte[48];
	...
	private static byte[] partL_bite(byte[] b,int j) {// 获得左半边
		byte a[] = new byte[j];
		for (int i = 0; i<j ; i++) {
			a[i] = b[i];			
		}
	...
	private static byte[] partR_bite(byte[] b,int j) {// 获得右半边
		int m=0;
		byte a[] = new byte[j];
		for (int i =0; i <j ; i++) {
			  a[i] = b[i+j];  
	...
	private static byte[] permute_pc1(byte[] b) {// PC1置换
		byte a[] = new byte[56];
	...
	private static String re_miwen(byte[] b) {//
		int i=0;
		byte[] inm = b;
	 ...
	 private static void dencryptByBlock(String txt,byte []b,int k){  
         int i, j;   
	 ...
	 private static byte[] re_ming(String m_txt,int len) throws UnsupportedEncodingException  //解密
	    {  
	...
	private static byte[] re_key(String kstr) throws UnsupportedEncodingException{//得到16次秘钥，64经过pc1变成56位，分成两部分，对两部分分别
	...
	private static String group(String str) throws UnsupportedEncodingException{
		String miwen="";
		String mingwen="";
		int length=0;
		int index=0;
	...
	//	System.out.println("我是解密后明文：:"+jie_mi);
	 ...
	 private static String dencrypt(String m_txt,int len)  
	    {  
	        byte b_bit[] = new byte[64],a_byte[]= new byte[8],c_byte[] = new byte[len];  
	        int i=0,j=0,k=0;  
	        for(i=0;i<m_txt.length();i=i+16)  
	...
	/* public void jiemi(String str) throws UnsupportedEncodingException{
		 System.out.println("我是密文："+str);
		 System.out.println("i am:"+dencrypt(str,8));
		 /*byte[] by_all=str.getBytes();
		String jie_mi=new String(by_all);
		System.out.println(jie_mi);
		
	}
	public String jiami(String mstr,String kstr) throws UnsupportedEncodingException {	
				
	    return group(mstr);
	}*/
	 
	//加解密，1加密，2解密
	public static String des(String mstr,String kstr,int i) throws UnsupportedEncodingException
	{
		re_key( kstr);
		String str = null;
		if(i==1)
		...
		{		
//			System.out.println("我是密文："+mstr);
			str=dencrypt(mstr,1000);
		}
		return str.trim();
	}
	//密钥生成
